...
func main() {
	...
	// Créez un WaitGroup pour attendre la fin du serveur HTTP
	var wg sync.WaitGroup
	wg.Add(1)

	go func() {
		//Commande pprof à réaliser après avoir lancé le programme : go tool pprof http://localhost:8080/debug/pprof/profile?seconds=30
	...
	go consumeCPUperlinGenerator(gradients)
	...
	// Attendez que le serveur HTTP soit terminé
	fmt.Println(`Press "Ctrl + C" to cancel listening...`)
	wg.Wait()
}

func consumeCPUperlinGenerator(gradients [][][]float64) {
	var i int
	for {
		// Créez une nouvelle image
		img := image.NewRGBA(image.Rect(0, 0, 800, 800))
